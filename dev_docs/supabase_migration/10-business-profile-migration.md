...
## Objective
Migrate the business profile service from Firestore to Supabase Postgres.

## Steps

### 1. Update `src/services/businessProfileService.ts`
...
1. **Document reference**: 
   - Firestore: `doc(db, 'accounts', accountId, 'businessProfile', 'profile')`
   - Supabase: `supabase.from('business_profiles').eq('account_id', accountId)`

2. **Upsert logic**: 
   - Firestore: `setDoc(..., { merge: true })`
   - Supabase: Check existence, then insert or update

3. **Field names**: Convert camelCase to snake_case

## Verification
- [ ] Can get business profile
- [ ] Can create business profile
- [ ] Can update business profile
- [ ] Logo URL handling works
