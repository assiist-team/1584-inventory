...
async function migrateTransactions() {
  try {
    ...
    for (let i = 0; i < dataRows.length; i++) {
      ...
      const transaction = mapCSVToTransaction(values, headers, projectId);
      if (transaction) {
...
function mapCSVToTransaction(values, headers, projectId) {
  ...
  const transaction = {
    ...
    created_by: 'migration-script', // You might want to change this
  ...
  headers.forEach((header, index) => {
    if (fieldMap[header] && values[index]) {
      ...
      // Skip amount fields as we already handled them above
      if (fieldName !== 'client_amount' && fieldName !== 'amount_1584') {
  ...
  if (transaction.receipt) {
    if (transaction.notes) {
      transaction.notes += ` | Receipt: ${transaction.receipt}`;
    } else {
      transaction.notes = `Receipt: ${transaction.receipt}`;
    }
    transaction.receipt_image = transaction.receipt; // Store receipt reference
  }
